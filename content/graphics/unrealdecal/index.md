...
\
But we can represent a low precision floating point number with 16 bits using <a class="link" href="https://en.wikipedia.org/wiki/Half-precision_floating-point_format"> IEEE 754 Half Precision </a>. And then the theory behind what we need to do is not too complicated. Separate the lower and upper 8 bits of each float and write them into their own Dbuffer Channel. In my implementation of passing in UV Coordiates, I end up breaking it up like this:
